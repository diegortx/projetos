...
<div class="card text-center">
 ...
 <?php 
        $result_Windows = "SELECT * FROM registromaquinas WHERE maq_tipo = 'WINDOWS' ";
        $maq_windows1 = mysqli_query($conn, $result_Windows);
        $maq_windows = mysqli_num_rows($maq_windows1);
           
        
        $result_Linux = "SELECT * FROM registromaquinas WHERE maq_tipo = 'LINUX' ";
        $maq_linux1 = mysqli_query($conn, $result_Linux);
        $maq_linux = mysqli_num_rows($maq_linux1);
        

        $result_Impressora = "SELECT  * FROM registromaquinas WHERE maq_tipo = 'IMPRESSORA' ";
        $impressoras1 = mysqli_query($conn, $result_Impressora);
        $impressoras = mysqli_num_rows($impressoras1); 

        $result_Servidor = "SELECT * FROM registromaquinas WHERE maq_tipo = 'SERVIDOR' ";
        $servidores1 = mysqli_query($conn, $result_Servidor);        
        $servidores =  mysqli_num_rows($servidores1);

        $maq_total = ($maq_windows +  $maq_linux + $impressoras +  $servidores);

   ?>

   <p class="h5">Total de Maquinas registradas : <?php echo $maq_total?> </p>
...
echo $resultado; -->
   ...
   <tbody>  
       <tr>  
       <td><?php echo $maq_windows ?></td>
       <td><?php echo $maq_linux ?></td>
       <td><?php echo $impressoras?></td>
